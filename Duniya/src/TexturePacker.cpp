...
void TexturePacker::AddTexture(uint32_t entity) {
    ...
    if (k == nullptr) throw std::exception();
...
void TexturePacker::SetScene(Scene* scene) { this->scene = scene; }
...
// void TexturePacker::Util(T& rects, K& guide, uint8_t* data, Heuristic
// heuristic)
//{
//	while(!rects.empty())
//	{
...
//			if((i + j) >= rowSize)
//				throw std::exception();
//			auto plusMargin = data + guide[i] + margin + (colSize * (j
//+ i + margin)); 			std::copy(tmpData, tmpData + width,
// plusMargin);
//		}
//	}
// }
...
uint8_t* PacKToBuffer(std::priority_queue<std::pair<T, T>, U>& rects,
		      std::vector<Vect4>& uvs,
		      std::function<uint8_t*(const U&)>& retrieveFunction,
		      std::function<uint64_t(const U&)>& toUV,
		      const uint64_t& width, const uint64_t& height,
		      uint8_t channel) {
    auto atlasPtr = new uint8_t[width * height * channel];
    std::vector<std::vector<bool>> guide(width,
	...
	if (start == -1 || alignment == -1) return nullptr;
	start += margin;
	alignment += margin;
	for (int i = 0; i < chHeight; i++) {
	    auto already = chWidth * i;
	    auto alreadyData = width * (start + i);
	    std::copy(glympBitmap + already, glympBitmap + already + chWidth,
		      atlasPtr + alreadyData + alignment);
	    for (int a = 0; a < chWidth; a++)
		guide[start + i][alignment + a] = true;
	}
	auto high = (float)chHeight / (float)height;
	uvs[toUV(k.second)] = Vect4((float)alignment / (float)width,
				    (float)start / (float)height + high,
...
uint32_t TexturePacker::Pack() {
	...
	while ((guide[j] + texWidth) > width) j++;

	for (int i = 0; i < texHeight; i++) {
	    if ((i + j) >= height) throw std::exception();
	    auto plusMargin =
		data + guide[i] + margin + (width * (j + i + margin));
...
int32_t TexturePacker::PackFont(std::string fontFile, FontDict& dict,
				uint32_t fontSize) {
    // std::priority_queue<std::array<int32_t, 3>,
    // std::vector<std::array<int32_t, 3>>, RectComp<std::array<int32_t, 3>>>
    // rects;
    auto scene = GetScene();
    std::priority_queue<std::array<int32_t, 3>,
			std::vector<std::array<int32_t, 3>>>
	rects;
    std::vector<std::vector<bool>> guide(height,
	...
	switch (error) {
		...
		throw std::runtime_error(
		    "something went wrong during font file \n error no:" +
		    std::to_string(error));
	};
    }
    if (face == nullptr) std::runtime_error("face is null");
	...
	while (chIndex) {
	    ...
	    rects.push({(int)glymp->bitmap.width, (int)glymp->bitmap.rows,
			(int32_t)c});
	...
	// if((end - j) < chHeight) break;
	if (start == -1 || alignment == -1) return -1;
	start += margin;
	alignment += margin;
	for (int i = 0; i < chHeight; i++) {
	    auto already = chWidth * i;
	    auto alreadyData = width * (start + i);
	    std::copy(glympBitmap + already, glympBitmap + already + chWidth,
		      data + alreadyData + alignment);
	    for (int a = 0; a < chWidth; a++)
		guide[start + i][alignment + a] = true;
	}
	auto high = (float)chHeight / (float)height;
	dict.glyps[ch].uv = Vect4((float)alignment / (float)width,
				  (float)start / (float)height + high,
				  (float)chWidth / (float)width, -high);
    ...
    stbi_write_png("fontBitmap.png", width, height, 1,
		   (*scene->resourceBank).resources[dict.texture.data].Get(),
		   width);
    return 0;
}
