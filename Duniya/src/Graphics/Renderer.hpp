...
enum class Options { BLEND, DEPTH_TEST, FACE_CULL, WIREFRAME_MODE };

struct GBuffer {
    struct GBufferStyle {
	enum class CPUFlags { STATIC, DYNAMIC, STREAM } cpuFlags;
	enum class Usage { DRAW, COPY, READ } usage;
	enum class BufferType { VERTEX, INDEX } type;
    } bufferStyle;
    uint32_t count;
    uint32_t sizet;
    uint32_t data;
...
class Renderer {
   ...
   public:
    ...
    virtual void DrawInstanced(DrawPrimitive drawPrimitive, GBuffer* gBuffer,
			       uint32_t numInstanced) = 0;
    virtual void DrawBuffer(DrawPrimitive drawPrimitive, GBuffer* gBuffer) = 0;
    virtual void DrawArrays(DrawPrimitive drawPrimitive, GBuffer* gBuffer,
			    uint32_t numElement) = 0;
    virtual void DrawInstancedArrays(DrawPrimitive drawPrimitive,
				     GBuffer* gBuffer, uint32_t numElement,
				     uint32_t numInstanced) = 0;
    ...
    virtual void Uniform1f(const uint32_t count, const float* data,
			   std::string name) = 0;
    virtual void Uniform2f(const uint32_t count, const Vect2* data,
			   std::string name) = 0;
    virtual void Uniform3f(const uint32_t count, const Vect3* data,
			   std::string name) = 0;
    virtual void Uniform1u(const uint32_t count, const uint32_t* data,
			   std::string name) = 0;
    virtual void Uniform1i(const uint32_t count, const int32_t* data,
			   std::string name) = 0;
    virtual void Uniform4f(const uint32_t count, const Vect4* data,
			   std::string name) = 0;
    virtual void UniformMat(const uint32_t count, const Mat* mat,
			    std::string name) = 0;
    ...
    virtual uint32_t AddSpecification(
	VertexSpecification& vertexSpecification) = 0;
};
